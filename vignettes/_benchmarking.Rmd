...
# Simulate network
network <- sim_sbm_network(group_info, edge_propensities, edge_dist = purrr::rbernoulli)
...
collapse_results <- create_sbm(network) %>% 
  collapse_blocks(exhaustive = TRUE, 
                  num_block_proposals = 8, 
                  num_mcmc_sweeps = 15)
```
...
library(tictoc)
tic()
collapse_run(my_sbm, end_block_num = 10, sigma = 5)
toc()

tic()
collapse_run(my_sbm, end_block_num = 10, sigma = 5, parallel = TRUE)
toc()
```
...
pkg_sequential <- function(){
  collapse_run(my_sbm, 
               start_block_num = low_num,
               end_block_num = high_num, 
               sigma = sigma, 
               num_block_proposals = num_block_proposals,
               num_mcmc_sweeps = num_mcmc_sweeps,
               parallel = FALSE)
