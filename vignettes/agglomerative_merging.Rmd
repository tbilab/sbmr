...
plot_merge_results <- function(merge_results_df, group_cutoff = 30){
  ...
  ggplot(merge_results_df, aes(x = n_groups, y = entropy)) +
    geom_line(alpha = 0.3) +
    geom_point(size = 0.5) +
...
```{r}
merges_over_sweeps <- purrr::map_dfr(
  seq(1, 50, by = 5),
  ~{
    new_sbm <- create_sbm(edges, nodes)
    merge_results <- new_sbm$collapse_groups(0L, as.integer(.x))
    results_to_df(merge_results) %>% mutate(mcmc_sweeps = .x)
  })


plot_merge_results(merges_over_sweeps) + 
  facet_wrap(~mcmc_sweeps) +
  ggtitle("Tuning number of mcmc sweeps per stage")
```


```{r}
merges_over_sweeps_and_greedy <- expand.grid(
  mcmc_sweeps = seq(1, 51, by = 10),
  greedy = c(TRUE, FALSE)
) %>% 
  {
    purrr::map2_dfr(
