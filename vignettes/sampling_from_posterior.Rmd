...
sweep_results <- mcmc_sweep(my_sbm, num_sweeps = num_sweeps)

sweep_df <- tibble(entropy_delta = sweep_results$entropy_deltas) %>% 
  mutate(sweep = 1:n(),
         label = 'eps = 0.1')
...
nodes_w_inferred_block %>% 
  group_by(inferred) %>% 
  summarise(n_unique_blocks = length(unique(block)))
```
...
sweep_df_lower_eps <- tibble(
    entropy_delta = mcmc_sweep(my_sbm, num_sweeps = num_sweeps)$entropy_deltas
  ) %>% 
  mutate(sweep = 1:n(),
