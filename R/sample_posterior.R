...
sample_posterior <- function(
  ...
  sbm <- create_sbm(simulated_network$edges, simulated_network$nodes)

  # Set model parameters
  sbm$GREEDY <- greedy
  sbm$BETA <- beta
  sbm$EPS <- epsilon
  sbm$N_CHECKS_PER_GROUP <- n_checks_per_group

  # # Run initialization algorithm on base level nodes
  # merge_results <- sbm$collapse_groups(0, as.integer(initialization_mcmc_steps))
  #
  # tibble(
