...
template <typename T>
class Ordered_Ptr_Pair {
  private:
  T* val_1;
  T* val_2;

  public:
  Ordered_Ptr_Pair(const T* a, const T* b)
      : val_1(a < b ? a : b)
      , val_2(a < b ? b : a)
  {
  }
...
template <typename T>
bool operator==(const Ordered_Ptr_Pair<T>& a, const Ordered_Ptr_Pair<T>& b)
{
...
template <typename T>
struct Ordered_Ptr_Pair_Hash {
  size_t operator()(const Ordered_Ptr_Pair<T>& p) const
  {
...
template <typename T>
using Ordered_Ptr_Pair_Set = std::unordered_set<Ordered_Ptr_Pair<T>, Ordered_Ptr_Pair_Hash<T>>;
