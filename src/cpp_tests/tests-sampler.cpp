...
TEST_CASE("Sampling from nested vectors", "[Sampler]")
{
  ...
  for (int i = 0; i < n_samples; i++) {
    ...
    if (sampled_int == 4) n_times_4++;

    if (sampled_int == 3) n_times_3++;
  }

  const double proportion_4 = double(n_times_4) / double(n_samples);
  const double proportion_3 = double(n_times_4) / double(n_samples);
...
TEST_CASE("Node list and vector sampling", "[Sampler]")
{
  ...
  REQUIRE(times_list_agreed == n_samples);
  REQUIRE(times_vec_agreed == n_samples);
}
...
TEST_CASE("Node vector shuffling respects seeds", "[Sampler]")
{
  ...
  Node_Vec nodes_vec2;
  Node_Vec nodes_vec3;
  nodes_master.reserve(n_nodes);
  nodes_vec1.reserve(n_nodes);
  nodes_vec2.reserve(n_nodes);
  nodes_vec3.reserve(n_nodes);
  ...
  for (int i = 0; i < n_samples; ++i) {
    ...
    int n_should_match   = 0;
    int n_shouldnt_match = 0;
    ...
    REQUIRE(n_should_match == n_nodes);
    REQUIRE(n_shouldnt_match < n_nodes);
  }
}
